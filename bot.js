...
bot.hears('📺 Daily Ads', async (ctx) => {
  ...
  // ✅ FIX: Markup.button.webApp (capital A - Telegraf v4)
  await ctx.reply(
    `📺 *DAILY ADS*\n\n` +
...
bot.hears('🎡 Daily Bonus', async (ctx) => {
  ...
  await ctx.reply(
    ...
    `👇 Open the spin wheel below:`,
    {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.webApp('🎡 Spin the Wheel!', `${CONFIG.GITHUB_PAGES_URL}?mode=spin&userId=${user.telegramId}&spinMode=${spinMode}&streak=${user.spinStreak}`)]
      ])
    }
  );
});
...
bot.hears('🏆 Sunday Tournament', async (ctx) => {
  ...
  if (!user.hasUnlockedUnlimitedSunday && user.sundayAdsCount < 20) {
    keyboard.push([Markup.button.webApp('📺 Watch Sunday Ads', `${CONFIG.GITHUB_PAGES_URL}?mode=sunday&userId=${user.telegramId}`)]);
  }
  ...
  if (user.hasUnlockedUnlimitedSunday) {
    keyboard.push([Markup.button.webApp('📺 Watch Unlimited Ads', `${CONFIG.GITHUB_PAGES_URL}?mode=sunday&userId=${user.telegramId}`)]);
  }
...
bot.action('unlock_sunday', async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    'Keep climbing the leaderboard! 🚀',
    {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.webApp('📺 Watch Unlimited Ads', `${CONFIG.GITHUB_PAGES_URL}?mode=sunday&userId=${user.telegramId}`)],
...
bot.hears('ℹ️ Help & FAQ', async (ctx) => {
  ...
  await ctx.reply(
    ...
    `⚙️ *Bot Version:* 2.1.0 | Made with ❤️`,
    { parse_mode: 'Markdown' }
  );
});
  ...
  for (const u of users) {
    try {
      if (ctx.message.photo) {
        ...
        await ctx.telegram.sendPhoto(u.telegramId, photo, { caption: ctx.message.caption || '' });
      } else if (ctx.message.video) {
        await ctx.telegram.sendVideo(u.telegramId, ctx.message.video.file_id, { caption: ctx.message.caption || '' });
...
// ✅ NEW: POST /api/reward - Universal reward endpoint for frontend
// Accepts: { userId, coins, type } — type: 'ad' | 'spin' | 'sunday'
app.post('/api/reward', async (req, res) => {
  try {
    const { userId, coins, type } = req.body;

    if (!userId || !coins || !type) {
      return res.status(400).json({ success: false, error: 'Missing required fields: userId, coins, type' });
    }

    const coinsToAdd = parseInt(coins, 10);
    if (isNaN(coinsToAdd) || coinsToAdd <= 0 || coinsToAdd > 1000) {
    ...
    if (type === 'ad') {
      ...
      if (user.dailyAdsViewed >= 10) {
        return res.status(429).json({ success: false, error: 'Daily ad limit reached (10/10)' });
      }
    ...
    if (type === 'sunday') {
      if (!user.hasUnlockedUnlimitedSunday && user.sundayAdsCount >= 20) {
        return res.status(429).json({ success: false, error: 'Sunday free limit reached. Unlock unlimited first.' });
      }
    ...
    if (type === 'spin') {
      ...
      if (lastSpin && (now.getTime() - lastSpin.getTime()) < 23 * 60 * 60 * 1000) {
        return res.status(429).json({
          ...
          nextSpin: new Date(lastSpin.getTime() + 24 * 60 * 60 * 1000).toISOString()
        });
      }
      user.coins += coinsToAdd;
      ...
      // Streak logic
      if (lastSpin) {
        const daysSince = Math.floor((now.getTime() - lastSpin.getTime()) / (24 * 60 * 60 * 1000));
        user.spinStreak = daysSince <= 2 ? (user.spinStreak % 7) + 1 : 1;
      } else {
        user.spinStreak = 1;
      }
    ...
    return res.status(400).json({ success: false, error: 'Unknown type. Use: ad | spin | sunday' });
...
// POST /api/spin (legacy - kept for compatibility)
app.post('/api/spin', async (req, res) => {
  try {
    ...
    await user.save();
    return res.json({ success: true, coinReward, totalCoins: user.coins, spinStreak: user.spinStreak, isMegaSpin: user.spinStreak >= 7, nextSpin: new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString() });
  } catch (e) {
    return res.status(500).json({ error: 'Server error' });
  }
});

// POST /api/ads-callback (legacy - kept for compatibility)
  ...
  try {
    ...
    if (isSundayNow) {
      if (!user.hasUnlockedUnlimitedSunday && user.sundayAdsCount + adsCount > 20) {
        return res.status(400).json({ error: 'Sunday free limit reached.' });
      }
    ...
    return res.json({ success: true, reward: totalReward, totalCoins: user.coins, dailyAdsViewed: user.dailyAdsViewed, sundayAdsCount: user.sundayAdsCount, remaining: isSundayNow ? (user.hasUnlockedUnlimitedSunday ? 'Unlimited' : 20 - user.sundayAdsCount) : 10 - user.dailyAdsViewed });
  } catch (e) {
    return res.status(500).json({ error: 'Server error' });
  }
});
...
setInterval(async () => {
  try {
    ...
    if (now.getDay() === 1 && now.getHours() === 0 && now.getMinutes() < 5) {
      const winner = await User.findOne({ sundayAdsCount: { $gte: 30 } }).sort({ sundayAdsCount: -1 });
      ...
      console.log('🏆 Sunday tournament ended. Winner:', winner?.username || 'None');
    }
  } catch (e) {
...
console.log('========================================');
console.log('🚀 CYBERPUNK EARNING BOT v2.1 - FULLY OPERATIONAL');
console.log('========================================');
